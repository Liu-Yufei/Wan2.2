...
def _parse_args():
    ...
    parser.add_argument("--csv_file_name", type=str, default='/storage/pmj/data_paul/Wan2.1/t2v_pair_prompts_8.csv', help="The file of the prompts")
...
def generate(args):
    ...
    nums,prompts=load_prompts_from_csv(csv_file_name)
    output_file = csv_file_name.split("_")[-1].split('.')[0]
    save_file = f"{args.save_file}/output_{output_file}_{formatted_time}"
    ...
    #         offload_model=args.offload_model)
        ...
        if rank == 0:
            ...
            save_video(
                tensor=video[None],
                save_file=f"{save_file}/{nums[i]}.mp4",
                fps=cfg.sample_fps,
                nrow=1,
                normalize=True,
                value_range=(-1, 1))
